...
#' @param exon.remove.thrsh Threshold to return NULL if number of exons after QC is less than this number (default = 30)
...
plotTcellExTRECT <- function(vdj.region.df, exons.selected,
  ...
  if(dim(vdj.logR.df)[1] == 0 | length(exon.remove) > exon.remove.thrsh){
    return(NULL)}
