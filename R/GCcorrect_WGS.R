...
#' @param gene.fasta.start The chr position the fasta file begins
#' @importFrom stats lm
#' @return data frame of GC correct logR values
#' @name GCcorrect_WGS

GCcorrect_WGS <- function(tumour.logR, exons , exonList, gene.fasta, gene.fasta.start,
                          hg19_or38 = 'hg38',sliding = 1000){

  # Solve visible binding issue:
  pos <- GC <- exon.gc <- smooth.gc <- Ratio.gc.correct <-  NULL
  ...
  # summary(gc.lm)
  gc.lm.df <- data.frame(pos = tumour.logR$pos[as.numeric(names(gc.lm$residuals))],
                         Ratio.gc.correct = gc.lm$residuals)

  tumour.logR <- tumour.logR %>%
    dplyr::left_join(gc.lm.df, 'pos') %>%
    dplyr::filter(!is.na(Ratio.gc.correct))
